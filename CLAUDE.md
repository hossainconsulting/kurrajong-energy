...
Engagement workspace for the **Kurrajong Energy Pty Ltd** simulation, the
**Service Cloud Consultant** certification track. Hemayet plays the consultant; the
company and every record in it are fictional. See `README.md` for the brief and scope.

## The org

Target org alias **`kurrajong`**, a Developer Edition org authorised locally.

```bash
sf org display --target-org kurrajong
sf data query --target-org kurrajong --query "SELECT COUNT() FROM Account"
```
...
- **`.mcp.json`** starts the Salesforce DX MCP server pinned to the **`kurrajong`**
  ...
  `sf org login web --alias kurrajong` first.
